...
layui.config({
	base : "js/"
}).use(['bodyTab','form','element','layer','jquery','util'],function(){
	var form = layui.form(),
		layer = layui.layer,
	...
	//公告层
	function showNotice(){
		if(window.sessionStorage.getItem("notice") != null){
            layer.open({
                type: 1,
                title: "系统公告",
                closeBtn: false,
                ...
                btn: ['确定'],
                moveType: 1,
                content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p>'+window.sessionStorage.getItem("notice")+'</p></div>',
			...
			$.ajax({
				...
				success: function (result) {
					window.sessionStorage.setItem("notice",result.sysNotice);
					layer.open({
						type: 1,
						title: "系统公告",
						closeBtn: false,
