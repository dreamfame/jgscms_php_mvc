<?php
	...
	Class PhotoServer
	{
        ...
        public function GetShow(){
            $sql = "select photo.id,user.nickname as nickname,uid,des,photo.created_at,praise,comment,img1,img2,img3,img4,img5,img6,img7,img8,img9,verify,operator,top,private from ".$this->db_table." left join user on uid = openid where verify = 1 and private = 0 order by photo.top,photo.created_at desc limit 50";
            $result = $this->db->ExeSql($sql, $this->conn);
            return $result;
        }
        ...
        public function QueryPhoto($where){
            $sql = "select photo.id,user.nickname as nickname,uid,des,photo.created_at,praise,comment,img1,img2,img3,img4,img5,img6,img7,img8,img9,verify,operator,top,private from ".$this->db_table."  left join user on uid = openid".$where." order by photo.created_at desc";
            $result = $this->db->ExeSql($sql, $this->conn);
            return $result;
        }
